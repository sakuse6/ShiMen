...
**Excerpt**
```
| Claude Opus 4.8 | _(inherited 扩展智能体运行环境 session model)_ | Anthropic | Primary model (default for all ARS skills) |
...
...
```
| Claude (session model) | _(inherited 扩展智能体运行环境 session model — e.g., Fable 5)_ | Anthropic | Primary model (default for all ARS skills) |
...
...
**Why this is debt.** Internal inconsistency: the citation judge defaults to **gpt-5.5**-xhigh while the cross-model verification doc teaches **gpt-5.4** everywhere — and its `gpt-5.4*)` shell glob will *not* match a `gpt-5.5` value, so a user following the judge's default into `ARS_CROSS_MODEL` hits the unsupported-model warning. Local 隔离智能体运行环境 already defaults to gpt-5.5.
