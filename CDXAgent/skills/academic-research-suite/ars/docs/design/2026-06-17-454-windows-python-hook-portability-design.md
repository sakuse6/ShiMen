...
### Ground truth (official 扩展智能体运行环境 hook docs, verified this session)
- Shell-form hook runs via `sh -c` (macOS/Linux), Git Bash (Windows), or **PowerShell when Git Bash isn't installed**.
- `${CLAUDE_PLUGIN_ROOT}` is substituted by 扩展智能体运行环境 itself before the shell — the unexpanded value in the user's log is display text, not the executed command. (So "variable didn't expand" is NOT the bug.)
...
- **Core skill use (research / write / review) requires NO Python.** README prerequisites list only 扩展智能体运行环境 + API key; there is a `requirements-dev.txt` but no `requirements.txt`. SKILL.md/agent files are prompt/markdown that Claude reads.
