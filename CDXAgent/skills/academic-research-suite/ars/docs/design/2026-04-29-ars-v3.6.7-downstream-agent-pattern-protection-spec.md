...
When `bibliography_agent` flags an entry as "pending manual verification" (e.g., 待人工查证), the entry still exists in the corpus. Downstream `synthesis_agent` reads "entry exists" as "entry is verified," and writes declarative claims about that entry's content as fact.
