...
- Create: `scripts/adapters/examples/folder_scan/input_fixture/中文档名_2024.pdf` (empty file — will be rejected: unicode)
...
touch "scripts/adapters/examples/folder_scan/input_fixture/中文档名_2024.pdf"
...
v3.6.4 时没有任何 ARS agent 读取 `literature_corpus[]`。此栏位仅为 input port 定义，消费端整合（agent 实际使用语料做研究规划 / citation 生成 / ...）留待 v3.6.5+。
```
...
🤖 Generated with [扩展智能体运行环境](https://claude.com/claude-code)
EOF
)"
```
