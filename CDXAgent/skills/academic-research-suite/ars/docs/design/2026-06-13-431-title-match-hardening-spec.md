...
**Decision authority:** owner-confirmed scope (this session: "全三层一次到位" + "negation veto 纳入,但保守"; v2 = "改" after the adversarial review)
...
| F3 | `title_search` returns only `scored[0]`; a wrong high-ratio #1 that fails metadata hides a correct lower-ratio #2 ⇒ real work dropped to unresolvable | ✓ (`crossref_client.py:203` `return scored[0][0]`) | **Gate moves INTO the candidate loop** (§0.2) — the resolver-layer落点 in the v1 body is wrong |
...
pass the single-candidate path — the exact落点 error round 1 already caught.
...
The earlier `endswith` design was rejected for exactly the误伤 this byte-equality form
