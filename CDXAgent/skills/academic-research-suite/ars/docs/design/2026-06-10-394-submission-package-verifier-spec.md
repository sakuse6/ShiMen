...
1. zh-TW self-citation phrasing list (A5) needs first-party curation — no anglophone-only pattern list. **CURATED (maintainer, 2026-06-10):** the slice-3 draft six (我们先前的研究/我们过去的研究/我们先前曾/我们已于先前/笔者先前的研究/本研究团队先前) confirmed, plus 本文作者先前 (third-person self-reference; anchored on the 本文 prefix because bare 作者先前 matches 该作者先前, a cited third party). Shortened catch-alls like 我们先前 rejected — they false-positive on within-paper back-references (the same reason the English list binds "we previously" to a verb). Extend in place as usage surfaces.
