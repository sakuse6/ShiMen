...
- **Source Han Serif TC VF** (思源宋体) — download from [Google Fonts](https://fonts.google.com/specimen/Noto+Serif+TC) or [Adobe GitHub](https://github.com/adobe-fonts/source-han-serif)
...
Do not install the whole repository as one nested skill folder under `.claude/skills/academic-research-skills/`; that buries the four `SKILL.md` files one level too deep for discovery. See Anthropic's [扩展智能体运行环境 Skills documentation](https://code.claude.com/docs/en/skills).

### Method 0: 扩展智能体运行环境 Plugin (v3.7.0+, recommended for 扩展智能体运行环境 CLI / IDE users)

If you use 扩展智能体运行环境 CLI, VS Code extension, or JetBrains extension, install ARS as a plugin:
...
- ✅ 扩展智能体运行环境 CLI / VS Code extension / JetBrains extension — full support
- ❌ claude.ai web / Claude for Work / Anthropic API direct calls — plugins not supported; use Method 1 / 2 / 3 below
- ➡️ 隔离智能体运行环境 — install the sibling distribution [`Imbad0202/academic-research-skills-codex`](https://github.com/Imbad0202/academic-research-skills-codex) (same workflow content, Codex-native packaging)

### Method 1: As project skills (recommended)

Use this when you want ARS available inside an existing 扩展智能体运行环境 project.
...
> **Global 扩展智能体运行环境 installation:** To make these skills available across your 扩展智能体运行环境 projects, install the four folders to `~/.claude/skills/` instead:
>
> ```bash
> git clone https://github.com/Imbad0202/academic-research-skills.git ~/academic-research-skills
>
...
> **Cowork does not read `~/.claude/skills/`.** That directory belongs to 扩展智能体运行环境 (the CLI / IDE), and Cowork does not scan it. Cowork loads skills you upload through **Settings → Capabilities → Skills**, each as its own zip. Symlinking or copying the skill folders into `~/.claude/skills/` will not make them appear in Cowork, no matter how many times you restart.
...
#### One trade-off versus 扩展智能体运行环境

Uploaded this way, each skill runs on its own as a standalone instruction set. This is a different experience from 扩展智能体运行环境. In 扩展智能体运行环境 the four skills work as a coordinated team: `academic-pipeline` chains them (research → write → review → revise) and each skill drives its own group of sub-agents. Cowork's uploaded-skill runtime does not provide that sub-agent orchestration, so the individual skills respond, but the full end-to-end pipeline does not run the way it does in 扩展智能体运行环境. For the full orchestrated experience, install ARS in 扩展智能体运行环境 via Method 0 (plugin) or Method 1 (project skills) above.

### Method 4: Use with claude.ai (web)

ARS is a 扩展智能体运行环境-native suite. The four skills are 12-13-agent teams that depend on multi-agent orchestration, executable scripts under `scripts/`, and Material Passport file handoffs. claude.ai's web interface delivers a different runtime than 扩展智能体运行环境, and the two access paths it offers reach this repository in different ways:
...
Use this when you want claude.ai to have access to the repo content — including the agent definitions, references, and example outputs — for reading and citation, without needing agentic skill execution. For agentic execution, use Method 3 (Cowork) on the desktop, or Methods 1-2 in 扩展智能体运行环境.
...
1. **ARS depends on 扩展智能体运行环境-only orchestration features**. Each ARS skill drives 12-13 specialised agents through 扩展智能体运行环境's Task / subagent tooling and Material Passport file handoffs that resume across sessions. The Anthropic-documented scope of claude.ai's Custom Skill runtime — a containerised code-execution environment per session, with the Skills user guide ([Use Skills in Claude](https://support.claude.com/en/articles/12512180-use-skills-in-claude)) describing skill activation but not multi-agent dispatch — does not include 扩展智能体运行环境's Task / subagent control surface. Method 4a is therefore expected to surface ARS as the SKILL.md body's instructions, without the multi-agent dispatch that produces the suite's actual outputs. We have not run a live upload to characterise this in detail; the recommendation is forward-looking based on the 扩展智能体运行环境-specific assumptions baked into the agent orchestration, not on a measured failure.
2. **Cost to 扩展智能体运行环境 and Cowork routing**. claude.ai limits each skill's `description` field to 200 characters per the [Custom Skills documentation](https://claude.com/docs/skills/how-to), while the [Agent Skills specification](https://agentskills.io/specification) and [扩展智能体运行环境 Skills documentation](https://code.claude.com/docs/en/skills) allow up to 1,024 characters. The four ARS descriptions currently sit in the 440-842 range, front-loading routing keywords that 扩展智能体运行环境 and Cowork use to discriminate between research, writing, review, and orchestration. Trimming them to fit Method 4a would weaken routing on 扩展智能体运行环境 and Cowork — the platforms ARS was built for — in exchange for an unverified partial fit on claude.ai.

**Recommended paths instead:**

- For skill execution on the desktop, use [Method 3 (Cowork)](#method-3-claude-cowork-desktop). The four skills upload as standalone Cowork skills; the multi-agent pipeline orchestration is only available in 扩展智能体运行环境 (Methods 0–2).
...
- For 扩展智能体运行环境 projects, use [Method 1 (project skills)](#method-1-as-project-skills-recommended) or [Method 2 (standalone)](#method-2-as-a-standalone-project).
...
**claude.ai vs 扩展智能体运行环境:**

- Method 4b is for content reading, not active Skill execution. For agentic skill execution, prefer Methods 1-3.
- claude.ai does not support local shell commands; results may be less comprehensive than 扩展智能体运行环境 workflows that rely on local scripts.
- Cross-model verification (`ARS_CROSS_MODEL`) requires 扩展智能体运行环境 with API keys.
- Direct `.docx` generation requires Pandoc, and LaTeX/PDF output requires 扩展智能体运行环境 with `tectonic`; claude.ai can still produce Markdown and DOCX conversion instructions.