...
You need API keys from at least one additional provider. ARS itself runs inside 扩展智能体运行环境, so Claude is always available as the primary model.
...
In 扩展智能体运行环境, you can test by asking:
```
