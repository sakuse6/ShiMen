...
## Command-Style Progress Protocol

Academic-paper must behave like a command-driven workflow.

When this workflow is active:

1. Begin every response with a progress block before any paper content.
2. The progress block must state:
   - Active skill: `academic-paper`
   - Current mode
   - Current phase or subphase
   - What has been completed so far
   - What is being done in this turn
   - What remains before the next gate
   - Whether user confirmation is required
3. Keep the progress block detailed enough that the user can see the paper
   pipeline state without reading the rest of the answer.
4. Treat `full`, `plan`, `outline-only`, `revision`, `revision-coach`,
   `abstract-only`, `lit-review`, `format-convert`, `citation-check`,
   `disclosure`, and `rebuttal-audit` as explicit command modes.
5. If the user's intent is ambiguous, report the ambiguity in the progress
   block and ask a concise routing question instead of guessing.
6. At each phase boundary or checkpoint, explicitly state the next gate and
   wait for user confirmation when the workflow requires it.
7. Preserve all existing hard gates, including citation, review, and revision
   constraints; progress reporting does not weaken them.
8. If the workflow is complete, report the final state and mark the progress
   block as complete.
...
**中文**: 写论文, 学术论文, 论文大纲, 写摘要, 修改论文, 文献回顾论文, 检查引用, 转 LaTeX, 转换格式, 研讨会论文, 期刊文章, 学位论文, 研究论文, 引导我写论文, 帮我规划论文, 逐步写论文, 写方法论, 写讨论, 审查意见, 修订路线图, 帮我修改, 我收到审查意见, 转换引用格式
...
Plan mode is a question-first planning workflow:

1. Keep asking targeted questions until each chapter or required section has
   enough information to draft cleanly.
2. Do **not** emit prose for the final article while chapter planning is still
   in progress, unless the user explicitly asks for a partial draft.
3. Record the user's answers as chapter-level decisions, insights, or constraints
   instead of turning them into article text immediately.
4. After all required chapters or sections have been fully planned, generate the
   complete article in one consolidated writing pass.
5. If a chapter remains unresolved, continue questioning rather than drafting
   around the gap.
6. Progress reports must show which chapter is being clarified and which chapter
...
### Chapter-First Writing Rule

For `plan` mode and any chapter-planning workflow, follow this rule:

1. Continue asking for missing information chapter by chapter.
2. Do not write the full article during the questioning/planning phase.
3. Store the answers as structured planning notes or chapter insights.
4. Once all required chapters are fully resolved, switch to a single end-stage
   drafting pass that produces the complete article at once.
5. If the user requests a partial draft for a specific chapter, that request must
   be explicit; otherwise, default to no-prose planning.
6. Keep the progress block aligned with the current chapter being clarified.
...
Follows the user's language. When the user writes in Chinese, output in Simplified Chinese by default. Academic terminology is kept in English. Bilingual abstracts are always provided regardless of the main text language.
