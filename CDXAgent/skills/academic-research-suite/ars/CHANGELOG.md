...
- PreToolUse hook (Phase 0.1 verified 扩展智能体运行环境 payload includes `agent_type` field; hook implementation requires multi-phase schema first — both deferred to v3.10).
...
- **`scripts/test_run_codex_audit_e2e.py`** — Phase 6.1 deferred end-to-end
  ...
  隔离智能体运行环境 via PATH-prefix shim emitting canonical Phase 2 JSONL stream.
...
> **扩展智能体运行环境 plugin packaging.** ARS now installs in one line on 扩展智能体运行环境
...
- **Treatment runs** (writer Phase 4a/4b + evaluator Phase 6a/6b on the seven
  ...
  Scholar API + 隔离智能体运行环境 runs; deferred to follow-up commits before the
...
- **`docs/SETUP.md` Method 4 (claude.ai) recommendation revised**. Method 4b
  ...
  - ARS depends on 扩展智能体运行环境-only orchestration features. Each skill drives
    12-13 specialised agents through 扩展智能体运行环境's Task / subagent tooling
    ...
    扩展智能体运行环境's Task / subagent control surface or cross-session Material
  ...
  - Trimming the four `description` fields below claude.ai's 200-character cap
    would weaken 扩展智能体运行环境 and Cowork routing on the platforms the suite was
    actually built for. The Agent Skills specification and 扩展智能体运行环境 Skills
    documentation both allow up to 1,024 characters; only claude.ai's upload
    UI enforces 200. Trading 扩展智能体运行环境 and Cowork routing precision for
...
- Doc-only patch. No `SKILL.md` (frontmatter or body), no agent file, no
  ...
  扩展智能体运行环境-native lengths (440-842 characters) so routing on 扩展智能体运行环境
...
- This patch is a scope change from the v3.6.5.2 originally forecast in the
  ...
  trim direction was abandoned because it would have damaged 扩展智能体运行环境 and
...
- **`docs/SETUP.md` Method 3 install paths** — Option A (symlink) and Option B (copy)
  ...
  buried the four `SKILL.md` files one level too deep for Cowork / 扩展智能体运行环境 discovery.
- **`docs/SETUP.md` Method 4 (claude.ai) restructured** — split into Method 4a
  ...
  their 扩展智能体运行环境-native lengths).
...
- **`resume_from_passport=<hash>` mode** in `academic-pipeline`. Lets users resume a pipeline run in a fresh 扩展智能体运行环境 session from the Material Passport ledger alone.
...
- `docs/PERFORMANCE.md` + `docs/PERFORMANCE.zh-TW.md` — per-mode token budgets, full-pipeline cost estimate, and recommended 扩展智能体运行环境 settings (Agent Team, Ralph Loop, Skip Permissions).
