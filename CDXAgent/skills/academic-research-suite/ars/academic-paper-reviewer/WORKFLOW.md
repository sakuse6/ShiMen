...
Academic-paper-reviewer must behave like a command-driven review workflow.

When this workflow is active:

1. Begin every response with a progress block before any review content.
2. The progress block must state:
   - Active skill: `academic-paper-reviewer`
   - Current mode
   - Current phase
...
3. Keep the progress block detailed enough that the user can see the review
   state at a glance.
4. Treat `full`, `re-review`, `quick`, `methodology-focus`, `guided`, and
   `calibration` as explicit command modes.
...
6. At the end of Phase 0 and at any mandatory checkpoint, explicitly state the
   next gate and wait for user confirmation when required.
7. Preserve all existing read-only and CRITICAL-threshold rules; progress
   reporting does not weaken them.
