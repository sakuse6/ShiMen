...
Deep-research must behave like a command-driven workflow, not an open-ended chat.

When this workflow is active:

1. Keep the current mode, submode, and phase visible in every response.
2. Begin every response with a progress block before any substantive content.
3. The progress block must state:
   - Active skill: `deep-research`
   - Active mode
   - Active phase or layer
   - What has been completed so far
   - What is being done in the current turn
   - What remains before the next gate
   - Any blocking question or confirmation
4. Treat `socratic`, `full`, `quick`, `review`, `lit-review`, `three-way-scan`,
   `fact-check`, and `systematic-review` as explicit command modes.
5. If the user intent is ambiguous, report the ambiguity in the progress block
   and ask a concise routing question instead of guessing.
6. During Socratic mode, the progress block should still appear, but the main
   content must remain question-led rather than answer-led.
7. At each phase boundary, explicitly state the next gate and wait for user
   confirmation when the workflow requires it.
8. If the workflow reaches completion, mark the progress block as complete and
...
**中文**: 研究, 深度研究, 文献回顾, 文献探讨, 系统性回顾, 后设分析, 证据综整, 事实查核, 三段式文献扫描, WHY HOW WHAT 论文比较, 研究方法, 学术分析, 政策分析, 引导我的研究, 帮我厘清, 监测这个主题, 设定追踪
...
| Your Situation 你的状况 | Recommended Mode | Spectrum |
|----------------|-----------------|----------|
| Vague idea, need guidance / 有模糊想法，需要引导 | `socratic` | originality |
| Clear RQ, need comprehensive research / 有明确 RQ，需要完整研究 | `full` | balanced |
| Need a quick brief (30 min) / 需要快速摘要 | `quick` | fidelity |
| Have a paper to evaluate before citing / 有论文需要评估 | `review` | balanced |
| Need literature review for a topic / 需要文献回顾 | `lit-review` | fidelity |
| Need a fast paper-comparison scan / 需要快速比较多篇论文 | `three-way-scan` | fidelity |
| Need to verify specific claims / 需要查核特定事实 | `fact-check` | fidelity |
| Need systematic review / meta-analysis / 系统性回顾或后设分析 | `systematic-review` | fidelity |
...
Follows the user's language. When the user writes in Chinese, output in Simplified Chinese by default. Academic terminology kept in English. Socratic mode uses natural conversational style.
