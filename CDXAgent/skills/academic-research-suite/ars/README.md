...
[简体中文版](README.zh-CN.md) | [繁体中文版](README.zh-TW.md) | [日本语版](README.ja-JP.md)

A comprehensive suite of 扩展智能体运行环境 skills for academic research, covering the full pipeline from research to publication.

**Install in 30 seconds** (扩展智能体运行环境 CLI / VS Code / JetBrains, v3.7.0+):
...
- [扩展智能体运行环境](https://docs.claude.com/en/docs/claude-code/setup) (latest; plugin packaging requires recent versions)
...
**👉 [docs/SETUP.md](docs/SETUP.md)** — full guide: install 扩展智能体运行环境, set up API keys, optional Pandoc/tectonic for DOCX/PDF, cross-model verification (`ARS_CROSS_MODEL`), and five installation methods (Plugin, project skills, global skills, claude.ai Project, repo-cloned).

**Using 隔离智能体运行环境?** Install the sibling distribution instead: [`Imbad0202/academic-research-skills-codex`](https://github.com/Imbad0202/academic-research-skills-codex) — same workflow content, Codex-native packaging as a single `$academic-research-suite` skill with `ars-*` aliases.

## Performance & cost

**👉 [docs/PERFORMANCE.md](docs/PERFORMANCE.md)** — per-mode token budgets, full-pipeline estimate (~$4–6 for a 15k-word paper), and recommended 扩展智能体运行环境 settings (Auto mode; Agent Team optional).
...
- **Traditional Chinese** (繁体中文) — default when user writes in Chinese
...
**Cheng-I Wu** (吴政宜) — Author and maintainer
...
### v3.7.0 (2026-05-05) — 扩展智能体运行环境 Plugin Packaging

> Plugin packaging upgrade: ARS now installs in one line on 扩展智能体运行环境 CLI / VS Code / JetBrains via `/plugin marketplace add Imbad0202/academic-research-skills` + `/plugin install academic-research-skills`. The traditional `git clone + symlink to ~/.claude/skills/` flow continues to work — both tracks are first-class.
...
- **Opt-in passport reset boundary** (`ARS_PASSPORT_RESET=1`). Promotes every FULL checkpoint to a context-reset boundary. New `resume_from_passport=<hash>` mode lets users resume in a fresh 扩展智能体运行环境 session from the Material Passport ledger alone. `systematic-review` mode with the flag ON makes reset mandatory at every FULL checkpoint; other modes treat reset as the flag-gated default. Flag OFF preserves pre-v3.6.3 behavior byte-for-byte.
...
- Added `docs/SETUP.md` (prerequisites, API keys, Pandoc/tectonic, cross-model verification, installation methods) and `docs/PERFORMANCE.md` (token budgets, recommended 扩展智能体运行环境 settings). README links to both instead of inlining them.
