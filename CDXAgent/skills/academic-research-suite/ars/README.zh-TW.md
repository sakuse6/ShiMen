# Academic Research Skills for 扩展智能体运行环境
...
[English](README.md) | [简体中文版](README.zh-CN.md) | [日本语版](README.ja-JP.md)

一套完整的学术研究 扩展智能体运行环境 技能包，涵盖从研究到论文出版的全流程。

**30 秒安装**（扩展智能体运行环境 CLI / VS Code / JetBrains，v3.7.0+）：
...
装完跑 `/ars-plan`，ARS 会用苏格拉底对话帮你规划章节结构。需要前置条件或传统 symlink 安装请看 [快速安装](#快速安装)。
...
Lu 等人（2026，*Nature* 651:914-919）发表的 **The AI Scientist** 是第一个端到端全自动的 AI 研究系统，其生成的论文通过 ICLR 2025 workshop 的盲审（评分 6.33/10，workshop 平均 4.87）。他们自己的 Limitations 段落也列出了这类系统会遇到的结构性失败模式：实作错误、幻觉实验结果、取巧特征依赖、实作错误被包装成「意外发现」、方法论伪造、框架锁定、引用幻觉。

ARS 建立在这个前提上：**人类研究者 + AI 的组合，比纯自动或纯人工都更能避开这些失败模式**。Stage 2.5 与 Stage 4.5 诚信闸门执行 7 类阻断式检查清单（见 [`academic-pipeline/references/ai_research_failure_modes.md`](academic-pipeline/references/ai_research_failure_modes.md)），reviewer 也提供 opt-in 的 calibration mode 用使用者自备的 gold set 测量 FNR/FPR。

[**Zhao 等人**](https://arxiv.org/abs/2605.07723)（2026-05）盘点了 arXiv、bioRxiv、SSRN、PMC 上 250 万篇论文里的 1.11 亿笔引用，保守估计 2025 年单年就有 146,932 笔幻觉引用，并观察到 2024 年中是上升的拐点；bioRxiv-to-PMC 这条配对的「预印本进到正式发表」幻觉存活率达 85.3%。他们把「真实引用被用来支撑被引文献其实没有提出的主张」描述为当前未解的问题。ARS v3.7.1 为来源 provenance 加上 trust-chain frontmatter，v3.7.3 为未来的 claim-level 稽核铺上 locator 基础建设（三层引用 anchor），并在引用时段带出 advisory 风险讯号（ARS 内部把这条 claim-faithfulness 缺口标记为「L3」，此为 ARS 的用词，不是论文的用词）。v3.7.x 的设计动机来自 Zhao 等人的 corpus-scale 发现；ARS 本身的 corpus-scale 评估仍是未来工作。

v3.8 补上 L3 缺口的另一半。v3.7.3 让每一笔引用都带 locator anchor，v3.8 在这个基础上加一道 opt-in 稽核（`ARS_CLAIM_AUDIT=1`）：抓回每一个 anchor 指向的原始文本，判断论文里的 claim 是否真有被该引用支撑。五类新的 HIGH-WARN annotation（claim-not-supported、negative-constraint-violation、fabricated-reference、anchorless、constraint-violation-uncited）会在 formatter terminal hard gate 直接拦下输出。Calibration 随 release 出 20 笔 gold set，采 FNR<0.15、FPR<0.10 双阈值；正式放大投入前要先有 calibration 证据（v3.8 spec §5）。

v3.3 的灵感来自 [**PaperOrchestra**](https://arxiv.org/abs/2604.05018)（Song, Song, Pfister & Yoon, 2026, Google）：Semantic Scholar API 验证、反泄漏协议、VLM 图表验证、分数轨迹追踪。

---

## 架构与 pipeline

**👉 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** — 完整 pipeline 视图：流程图、阶段 × 维度矩阵、资料存取流、skill 依赖图、品质闸门、模式清单。

这份架构文件取代了原本散在 README 各处的 pipeline 描述。关于「哪个阶段跑什么」的所有资讯都集中在一个地方。

## 快速安装

**前置条件**

- [扩展智能体运行环境](https://docs.claude.com/en/docs/claude-code/setup)（建议最新版；plugin packaging 需要近期版本）
- 已 export `ANTHROPIC_API_KEY`，或第一次跑 `claude` 时设定
- *选用：* Pandoc 用于 DOCX 输出，tectonic + 思源宋体 TC 用于 APA 7.0 PDF（纯 Markdown 输出两个都不需要）
...
**验证可用：** 跑 `/ars-plan` 并描述你正在写的论文，ARS 会用苏格拉底对话帮你规划章节结构。想要单次测试的话改跑 `/ars-lit-review "你的主题"`。

**👉 [docs/SETUP.zh-TW.md](docs/SETUP.zh-TW.md)** — 完整指南：安装 扩展智能体运行环境、设定 API key、选用的 Pandoc/tectonic（DOCX/PDF）、跨模型验证（`ARS_CROSS_MODEL`），以及五种安装方式（Plugin、专案 skills、全域 skills、claude.ai Project、repo clone）。

**用 隔离智能体运行环境？** 请改装姊妹版：[`Imbad0202/academic-research-skills-codex`](https://github.com/Imbad0202/academic-research-skills-codex)。同一套 workflow 内容，Codex 原生包装为单一 `$academic-research-suite` skill，提供 `ars-*` 别名。

## 效能与费用

**👉 [docs/PERFORMANCE.zh-TW.md](docs/PERFORMANCE.zh-TW.md)** — 各模式 token 预算、完整 pipeline 估算（一篇 15k 字论文约 ~$4–6），以及建议的 扩展智能体运行环境 设定（Auto 模式；Agent Team 选用）。
...
- **Deep Research** — 13 个 Agent 的研究团队，支援苏格拉底引导、PRISMA 系统性回顾、意图侦测、对话健康度监控、可选跨模型 DA、Semantic Scholar API 验证。
- **Academic Paper** — 12 个 Agent 的论文撰写团队，含风格校准、写作品质检查、LaTeX 输出强化、视觉化、修订教练、引用格式转换、反泄漏协议、VLM 图表验证。
- **Academic Paper Reviewer** — 7 个 Agent 的多视角同侪审查，0-100 品质量表（主编 + 3 位动态审查者 + 魔鬼代言人），含让步门槛协议、攻击强度保持、可选跨模型 DA critique / calibration、R&R 追溯矩阵、唯读约束。
- **Academic Pipeline** — 10 阶段全流程调度器，含自适应 checkpoint、宣称验证、素材护照、可选 `repro_lock`、可选跨模型诚信验证、中途强化机制、分数轨迹追踪。
- **资料存取层级标注**（v3.3.2+）— 每个 skill 宣告 `data_access_level`（`raw` / `redacted` / `verified_only`），由 `scripts/check_data_access_level.py` 强制执行。设计灵感来自 Anthropic 的 automated-w2s-researcher（2026）。详见 [`shared/ground_truth_isolation_pattern.md`](shared/ground_truth_isolation_pattern.md)。
- **任务类型标注**（v3.3.2+）— 每个 skill 宣告 `task_type`（`open-ended` 或 `outcome-gradable`）。目前 ARS 所有 skills 皆为 `open-ended`。
...
- **Artifact 可重现性 Lockfile**（v3.3.5+）— Material Passport 新增可选 `repro_lock` 子区块。**是设定文件化，不是重播保证** — LLM 输出不是位元可重现。详见 [`shared/artifact_reproducibility_pattern.md`](shared/artifact_reproducibility_pattern.md)。
- **实验来源凭证登录**（#260）— Material Passport 可选的 `experiment_provenance[]` 记录研究者在**外部**跑过的实验（ARS 从不执行实验），论文宣称透过 `claim_intent_manifest.planned_experiment_ids[]` 与之 join。诚信 gate（Stage 2.5/4.5）逐条比对实验支撑型宣称与登录凭证 — `ALIGNED` / `OVERSTATED` / `NOT_SUPPORTED_BY_PROVENANCE` / `PROVENANCE_INSUFFICIENT` — **但不判定实验本身是否正确**。fail-closed 的 `experiment_intake_declaration` 让「有没有跑实验」成为 Stage 1 明确决定。详见 [`shared/handoff_schemas.md`](shared/handoff_schemas.md)。
...
| [出版后稽核报告](examples/showcase/post_publication_audit_2026-03-09.pdf) | 独立全引用稽核：发现 21/68 篇问题，通过了 3 轮诚信审查仍漏网 |

---

## 搭配工具：Experiment Agent

如果你的研究需要在写作前跑实验（程式码或人工研究），[Experiment Agent](https://github.com/Imbad0202/experiment-agent) 技能填补 ARS Stage 1（研究）和 Stage 2（写作）之间的空缺。
...
**功能**：执行程式码实验（Python、R 等）并即时监控、管理人工研究 protocol 与 IRB 伦理审查、11 种统计谬误侦测、重现性验证。

**搭配使用方式**：ARS pipeline 跑完 Stage 1 后暂停，在另一个 experiment-agent session 中跑实验，完成后将结果（含 Material Passport）带回 ARS Stage 2。ARS 不需要任何修改。详见 [experiment-agent README](https://github.com/Imbad0202/experiment-agent)。
...
"把这批资料写成文献回顾论文"                  → lit-review mode（文献回顾论文）
"转换成 LaTeX" / "引用格式转 IEEE"            → format-convert mode（格式转换）
"检查引用格式"                                → citation-check mode（引用检查）
"帮我生成 NeurIPS 的 AI 使用揭露"             → disclosure mode（AI 揭露）
```
...
7 个 Agent 的多视角审查，搭配 **0-100 品质量表**。模式：full、re-review、quick、methodology-focus、guided、calibration。**决策对照：** ≥80 接受、65-79 小修、50-64 大修、<50 退稿。第一轮审查团队 vs. 精简再审团队的分界：见 ARCHITECTURE.md §3 Stage 3 / Stage 3'。

### Academic Pipeline (v3.13.0)

10 阶段调度器，含诚信验证、两阶段审查、苏格拉底指导、协作品质评估。Pipeline 保证：每个阶段都需使用者确认 checkpoint；诚信验证（Stage 2.5 + 4.5）不可跳过；R&R 追溯矩阵（Schema 11）独立验证作者修订宣称。v3.4 新增 Compliance Agent（PRISMA-trAIce + RAISE）于 Stage 2.5 / 4.5。v3.5 新增 **协作深度观察员**（`collaboration_depth_agent`，仅咨询性质、永不阻挡流程）于每一次 FULL/SLIM checkpoint 与 pipeline 完成时。MANDATORY 诚信闸门（2.5 / 4.5）明确跳过观察员，避免稀释合规检查。理论基础：Wang & Zhang (2026), IJETHE 23:11。逐阶段矩阵（agent、产出物、闸门）：见 ARCHITECTURE.md §3。
...
- **跨模型验证**：设定 `ARS_CROSS_MODEL` 启用第二 AI 模型独立审查。详见 [docs/SETUP.zh-TW.md](docs/SETUP.zh-TW.md)。
...
**[cloudenochcsis](https://github.com/cloudenochcsis)** — 贡献者。将 IS 章节从 *Basket of 8* 扩充为完整的 *Senior Scholars' Basket of 11*，补上 *Decision Support Systems*、*Information & Management*、*Information and Organization*（[Issue #7](https://github.com/Imbad0202/academic-research-skills/issues/7)、[PR #8](https://github.com/Imbad0202/academic-research-skills/pull/8)）。资料来源：[AIS Senior Scholars' List of Premier Journals](https://aisnet.org/research/seniorscholarsbasket/)。
...
> #133 收尾（hot-fix 层）。长期架构修正以 v3.10 active conductor 在 #134 追踪。新增：CLAUDE.md routing 厘清闸（跨 phase 素材 → 以 a-d 选项厘清，不静默 dispatch）、22 个 single-phase agent 加 prompt 硬 fence（`## Phase Boundary (v3.9.2)`）、16 个 multi-phase / phase-orthogonal / cross-phase-meta agent 刻意不加 fence（诚实 framing：纯 prose placebo 会造成假性 enforce 错觉）、advisory verifier `scripts/check_pipeline_integrity.py` 事后侦测 #133 pattern。Behavioral smoke test 含 cross-model spot-check（Opus 4.7 100% / Sonnet + GPT-5.5 ≥75%）。

### v3.9.1（2026-05-18）— #129 + #130 client hardening

> v3.9.0 hot-fix。包 OpenAlex / Crossref response-read 失败为 `*Unavailable`（#129）；`check_claim_audit_consistency` 对非字串 `manifest_id` 加 guard（#130）。无 spec 变动。

### v3.9.0（2026-05-17）— #102 跨索引三角测量

> #102 收尾。v3.7.3 已完成单索引（Semantic Scholar）污染侦测；v3.9.0 延伸至三索引三角测量（S2 + OpenAlex + Crossref），定位为**纯 advisory**。`contamination_signals` 新增两个 optional boolean（`openalex_unmatched`、`crossref_unmatched`）；manual-entry not-rule 对称延伸。Finalizer 加入 4-tier advisory matrix（k=0/1/2/3，计算范围为现有 `*_unmatched` 栏位），v3.7.3 的 legacy `CONTAMINATED-UNMATCHED`（k=1/k_max=1、S2-only case）保留。Formatter pass-through allowlist 从 3 条延伸至 9 条；refusal rules 1-10 依 R-L3-2-E 不变。Policy layer（strict modes、hard-block tier、`venue_type` / `triangulation_policy`）依 spec §2.3 延至 v3.10。k=3 marker 为 `CONTAMINATED-TRIANGULATION-UNMATCHED`（描述可观测现象，不推断成因）。新增 3 条 firm rules：R-L3-2-C（k 计算范围为现有栏位）、R-L3-2-D（不得 API 推断分类）、R-L3-2-E（refusal list 不扩充；pass-through allowlist 须与 finalizer 同步延伸）。

**迁移：** v3.7.3 corpus — 跑 `python scripts/migrate_literature_corpus_to_v3_9_0.py PATH` 补齐两个新栏位。pre-v3.7.3 corpus — **先**跑 `migrate_literature_corpus_to_v3_7_3.py`，再跑 v3.9.0 迁移工具（spec §3.7 daisy-chain；v3.9.0 工具只动已有 `contamination_signals.semantic_scholar_unmatched` 的 entries）。
...
- **Pipeline 整合**（`scripts/claim_audit_pipeline.py`）：line 1211-1224 的 swallow site 移除；`JudgeInvocationError` 改 emit UAF row + `continue` 到下个 (sentence, manifest) pair。`constraint_violations[]` 不会再被假 NOT_VIOLATED 污染。
- **Tests**：新增 18 笔（15 笔 schema/lint TSUAFUncitedAuditFailureInvariants + 3 笔 pipeline integration TP23UncitedJudgeOutageEmitsUAF）。Baseline 694 → 712 tests、0 regression。
- **Agent doc**（`academic-pipeline/agents/claim_ref_alignment_audit_agent.md`）：Output emission 表格新增第七列；Error handling 表格从 3 种 surface 扩成 4 种，新增 uncited 路径 UAF 列。

### v3.8.0（2026-05-16）— L3 Claim-Faithfulness Locator + Audit（配对 milestone）

> v3.7.3 + v3.8 端到端关闭 L3（claim-faithfulness）缺口。v3.7.3 铺 locator 基础建设（每笔引用都带三层 anchor，给未来的稽核抓得到原文位置）；v3.8 在这之上加一道稽核 pass，判断引用来源是否真的支撑论文的 claim，违反者在 formatter terminal hard gate 直接拦下。本次 release 也合并了从 v3.7.0 后累积的 5 个 audit-trail-shipped feature PR（#104 / #105 / #108 / #111 / #115）。

- **#103 — `claim_ref_alignment_audit_agent`**（v3.8 PR #121）：opt-in（`ARS_CLAIM_AUDIT=1`，预设 OFF）的 Stage 4→5 audit agent。对每笔抽样引用判断与原文段落是否一致，emit `claim_audit_results[]` + `claim_intent_manifests[]` + `claim_drifts[]` + `uncited_assertions[]` + `constraint_violations[]` 五个 aggregate。Finalizer 8 列 matrix 把 HIGH-WARN 类别（CLAIM-NOT-SUPPORTED / NEGATIVE-CONSTRAINT-VIOLATION / FABRICATED-REFERENCE / ANCHORLESS / CONSTRAINT-VIOLATION-UNCITED）导去 formatter REFUSE rules 6-10。Calibration runner 随 release 出 20 笔 gold set（T-C1 FNR<0.15 + FPR<0.10、T-C2 per-class、T-C3 shape integrity）。共 8 轮 dual-track review（R1 codex + Gemini 3.1-pro-preview、R2-R8 在 Gemini quota 用完后改 codex-only）；trajectory R1 4P1+2P2 → R8 0P1+4P2 ship gate。
- **v3.7.3 — Three-Layer Citation Emission + contamination signals**（PR #98）：`synthesis_agent` / `draft_writer_agent` / `report_compiler_agent` 加上 `## Three-Layer Citation Emission (v3.7.3)` H2。每个 `<!--ref:slug-->` 都带 `<!--anchor:<kind>:<value>-->`，`<kind> ∈ {quote, page, section, paragraph, none}`（quote anchor 限 25 字以内、值需 URL-encode）。`pipeline_orchestrator_agent` finalizer 升 5 cell 并加 precedence-zero NO-LOCATOR 检查。`formatter_agent` 在 hard gate 加上对 `[UNVERIFIED CITATION — NO QUOTE OR PAGE LOCATOR]` 的明确 refusal。`literature_corpus_entry.schema.json` 新增 optional 的 `contamination_signals: { preprint_post_llm_inflection, semantic_scholar_unmatched }` 物件，`bibliography_agent` 在 ingest 时计算两个讯号。11 轮 review trajectory（Codex×10 + Gemini cross-model×1）收敛 22 个 finding。Spec：`docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md`。外部动机：Zhao 等人 arXiv:2605.07723（2026-05）。
- **#108 — AI disclosure policy-anchor renderer**（2026-05-14）：在原本的 venue-track renderer 之外，新增 PRISMA-trAIce / ICMJE / Nature / IEEE 四条 policy-anchor disclosure 路径。
- **#111 — `slr_lineage` emission on systematic-review → academic-paper handoff**（2026-05-15）：Schema 9 新增 optional 的 boolean `slr_lineage` 栏位。Producer 是 `pipeline_orchestrator_agent`（每次 handoff transition 写入），consumer 是 `disclosure` mode（读到后按 §4.3 G2 invariant 路由到 `--policy-anchor=prisma-trAIce`）。
- **#104 — README motivation：Zhao 等人 corpus-scale 证据锚点**（2026-05-15）：README + `README.zh-TW.md` 动机段以 Zhao 等人 146,932 笔幻觉引用的发现作为 v3.7.x 线设计动机的证据锚点。
...
- **#115 — Semantic Scholar client 成熟度**（2026-05-15）：`scripts/semantic_scholar_client.py` 加 1 req/s throttle（侦测到 `S2_API_KEY` 时降到 0.1s）、URLError 触发的 outage latch、以及 `reset_outage_latch()` 给跨 passport 的长执行批次清算用。

### v3.7.0（2026-05-05）— 扩展智能体运行环境 Plugin 打包

> Plugin 打包升级：ARS 现可在 扩展智能体运行环境 CLI / VS Code / JetBrains 一行装（`/plugin marketplace add Imbad0202/academic-research-skills` + `/plugin install academic-research-skills`）。原本的 `git clone + symlink 到 ~/.claude/skills/` 安装流程不变、继续支援；双轨都是一级公民。

- **Plugin manifest 与 marketplace metadata**（Phase 1，PR #68）：`.claude-plugin/plugin.json` 宣告整个 suite（4 个 skill 透过 `skills/` 目录相对 symlink 自动探索）；`.claude-plugin/marketplace.json` 注册 plugin，使单一 GitHub-hosted endpoint 同时提供 marketplace listing 与 plugin 来源。README、`README.zh-TW.md`、`docs/SETUP.md` 都加入双轨安装指引。
- **10 个 slash command** 在 `commands/ars-*.md`（Phase 2.1，PR #69）将 `MODE_REGISTRY.md` 的条目对映到 `/ars-<mode>` 触发。每个 command frontmatter 钉住模型路由：`opus` 给 `full` 与 `revision-coach`（架构与审稿解读深度），`sonnet` 给其他 8 个。任何情境不用 Haiku。
- **3 个 plugin-shipped agent** 在 `agents/*_agent.md`（Phase 2.1，PR #69）以相对 symlink 指向 `deep-research/agents/` 内 v3.6.7 已 hardened 的下游 agent：`synthesis_agent`、`research_architect_agent`、`report_compiler_agent`。底线档名保留以对齐 `scripts/check_v3_6_7_pattern_protection.py` hard-pin 路径与 INV-3 manifest-confined Clause 1 不变式。Symlink（不复制）维持 single source of truth，避免 v3.6.7 §6 inversion sweep + INV-1/2/3 lint 已关闭的 Pattern C3 攻击面再开。（#413 起改为真实 byte-identical 复本：相对 symlink 在无 `core.symlinks` 的 Windows checkout 与 zip 下载安装会坏；single-source 保证改由 `scripts/check_agents_mirror_sync.py` byte-equality CI lint 承接。）
- **`model: inherit`** 加在这三个 source agent frontmatter 上。选 inherit 而非 pin `sonnet` 是为了让 Opus session 跑 ARS full pipeline 时 agent 仍是 Opus（不被降）。使用者的 `~/.claude/hooks/warn-agent-no-model.sh` PreToolUse hook 在派工边界已 gate Haiku，所以 inherit 解析到的是已经没 Haiku 的模型。
- **SessionStart announce hook** 在 `hooks/hooks.json` + `scripts/announce-ars-loaded.sh`（Phase 2.2，PR #70）。Plugin 载入时，hook 把 10 个 slash command、3 个 plugin agent、token 预算指引以 `additionalContext` 注入 LLM 第一轮。`startup` 与 `clear` 拿完整 announce；`resume` 与 `compact` 只拿一行确认，避免每次 resume 都烧 context。Bash 3.2 兼容 — macOS stock `/bin/bash` 直接跑，不需 `brew install bash`。
- **Phase 2.2 范围缩减**：原本规划的 `SubagentStop → run_codex_audit.sh` codex audit hook 在 v3.7.0 被排除，因为 (a) contract gap：SubagentStop payload 没带 stage / deliverable，wrapper 必要参数无法从 hook 推出；(b) invoker 边界：`run_codex_audit.sh` lines 4–7 明禁同 session in-LLM 呼叫，PostToolUse 在产出 deliverable 的 LLM session 内触发。真正的 audit-hook 整合留到后续版本，等 ARS 有 stage / deliverable propagation contract 再做。详见 `docs/design/2026-04-30-ars-v3.7.0-plugin-packaging-roadmap.md` Update note 2026-05-05（Phase 2.2 scope reduction）。
- **`docs/PERFORMANCE.md` + `.zh-TW.md`** 新增「v3.7.0 Plugin agent 与模型路由」节，说明 inherit 语意与目前 3-agent scope 边界。
...
- **没动的东西**：4 个 skill 目录、25 个 mode、agent prompt、schema 档案、lint contract 全不变。Plugin 打包只**新增**顶层介面（`commands/`、`agents/`、`hooks/`、`.claude-plugin/`、`skills/` symlink dir、3 个 source agent frontmatter 加 `model: inherit`）。既有 4.3k clone 安装用户完全不破。
...
> v3.6.6 因专案排序晚于 v3.6.7 才落地；design doc 内仍保留 v3.6.6 内部命名作为
> contract gate 版本，suite release 标 v3.6.8 维持 CHANGELOG 单调递增。

- **Schema 13.1**（`shared/sprint_contract.schema.json`）在 Schema 13 之上加两个 `mode` enum 值（`writer_full` + `evaluator_full`）、两个新 optional top-level 栏位（`pre_commitment_artifacts` writer-only、`disagreement_handling` evaluator-only）、12 条 `allOf` branch 强制 reviewer- / writer- / evaluator-conditional gate。既有 reviewer contract 在 Schema 13.1 下 byte-equivalent validate（§3.6 zero-touch promise）。
...
- **`academic-paper` SKILL + agent file 新增 `## v3.6.6 Generator-Evaluator Contract Protocol` 区块**（SKILL.md 101 行 + `draft_writer_agent.md` 47 行 + `peer_reviewer_agent.md` 57 行）。SKILL.md 另加 `## Known limitations` 区块承载 graceful-degradation + cross-session resume v3.6.7+ forward note。
- **Validator 扩充**：`scripts/check_sprint_contract.py` 做 SC-* mode-gating audit（SC-5 + SC-11 reviewer-only；SC-9 跨三个 mode family 各读对应栏位）。validator 单元测试从 54 条增加到 71 条（4 positive + 5 schema-branch negative + 2 §3.6 reviewer regression + 6 mode-gating）。
...
- **A/B evidence fixture stub**（`tests/fixtures/v3.6.6-ab/`，30 个档案）：manifest + README + 6 paper-A inputs/baseline + 1 paper-C inputs/baseline + Stage 3 reviewer excerpt + 6 codex-judge baseline placeholder。真实 fixture data 在后续 commit populate。
...
- **`shared/references/` 增加四份 reference 文件**：`irb_terminology_glossary.md`、`psychometric_terminology_glossary.md`、`protected_hedging_phrases.md`、`word_count_conventions.md`。protection 条款引用这些档案路径做为 operational contract。
...
- **PRE-SCREENED 可重现区块** 进 Search Strategy 报告：列出已纳入／排除／略过的 corpus entry，附 F3 zero-hit 注解与 F4a–F4f provenance 报告（针对 `obtained_via` / `obtained_at` 部分宣告情境）。`final_included = pre_screened_included[] ∪ external_included[]` 维持 neutral — bibliography entry 与 literature matrix row 不挂 provenance 标签。
- **Consumer 协定参考文件** 在 `academic-pipeline/references/literature_corpus_consumers.md`，包含 PRE-SCREENED 模板、BAD/GOOD 范例、四条 Iron Rule 与 per-consumer 读取指示。
- **CI lint** `scripts/check_corpus_consumer_protocol.py` 透过 manifest 驱动的 consumer 清单（`scripts/corpus_consumer_manifest.json`）强制九条协定不变式。
...
- 采 presence-based 启动，不变更 schema、不引入新 env flag。Parse 失败 fallback 到 external-DB-only flow，并 surface `[CORPUS PARSE FAILURE]`。`citation_compliance_agent` 的 corpus 整合延后（目标版本将于 v3.8 后再订）。
- 无破坏性变更，既有使用者 adapter 不需修改。

### v3.6.4（2026-04-25）— Material Passport `literature_corpus[]` 输入埠

- **Schema 9 新增 `literature_corpus[]`** 选填栏位作为使用者文献的输入埠。每笔 entry 符合 `shared/contracts/passport/literature_corpus_entry.schema.json`（CSL-JSON authors / year / title / source_pointer，加上 PRIVATE 选填的 `abstract` / `user_notes`）。
- **语言中性的 adapter 契约** 放在 `academic-pipeline/references/adapters/overview.md`：任何语言写的程式都能读使用者自己的 corpus source 并产出符合契约的 `passport.yaml` + `rejection_log.yaml`。Entry-level 错误 fail-soft、adapter-level 错误 fail-loud、输出顺序确定。
- **三个 reference Python adapter** 在 `scripts/adapters/`：`folder_scan.py`（档案系统的 PDF 资料夹）、`zotero.py`（Better BibTeX JSON export）、`obsidian.py`（vault frontmatter）。仅供起点参考；非 reference source 预期使用者自行实作 adapter。
...
- **仅输入埠**：v3.6.4 只定义 schema 与 adapter 契约，consumer 整合到 v3.6.5 才接上 `bibliography_agent` 与 `literature_strategist_agent`。
- 无破坏性变更。

### v3.6.3（2026-04-23）— 选用式 Passport 重置边界

- **Opt-in passport 重置边界**（`ARS_PASSPORT_RESET=1`）。把每个 FULL checkpoint 提升为 context 重置边界。新增 `resume_from_passport=<hash>` 模式，让使用者在新的 扩展智能体运行环境 session 单凭 Material Passport ledger 就恢复 pipeline，不重播先前对话。`systematic-review` 模式 flag ON 时，每个 FULL checkpoint 一律强制重置；其他模式视重置为 flag 开启后的强预设。Flag OFF 时 byte-for-byte 维持 pre-v3.6.3 行为。
- Schema 9 新增 append-only `reset_boundary[]` ledger，两种 entry kind（`kind: boundary` + `kind: resume`）。Hash 用 JSON Canonical Form + SHA-256，搭配 canonical placeholder 处理自我参照问题。选填 `pending_decision` 负责 MANDATORY 分支决策。
- 新 CI lint `scripts/check_passport_reset_contract.py`：任何提到 flag 的档案都必须指向权威协议文件。
- 协议文件：`academic-pipeline/references/passport_as_reset_boundary.md`。
...
- **两段 hard gate**：审稿人先在「论文内容盲」Phase 1 预先承诺评分计划，Phase 2 才看到论文；Phase 1 输出包在 `<phase1_output>...</phase1_output>` 资料分隔符内，缩窄 self-injection 面。
...
v3.5.1 新增 Socratic Mentor 的选用式诚实探测（设定 `ARS_SOCRATIC_READING_PROBE=1` 启用）。预设关闭。详见 CHANGELOG。

- **选用式阅读诚实探测**：设定 `ARS_SOCRATIC_READING_PROBE=1` 后，Socratic Mentor 在目标导向 session 中引用特定论文时，触发一次性诚实探测，请使用者摘述一段文字。拒绝回答仅记录，不扣分。探测结果写入研究计划摘要，并带入 Stage 6 AI 自我反思报告。不新增 agent，不变更 schema。
...
- **新增 agent**：`academic-pipeline` 新增 `collaboration_depth_agent`（Agent Team 从 3 成长为 4）。每个 FULL/SLIM checkpoint 与 pipeline 完成后（Stage 6 之后）触发，依 4 维度 rubric 对使用者与 AI 的协作模式评分。**纯观察建议，永不阻挡流程**。MANDATORY checkpoints（Stages 2.5 / 4.5 的完整性检查）**不**触发 observer，完整性闸门完全保留。
- **新增 rubric**：[`shared/collaboration_depth_rubric.md`](shared/collaboration_depth_rubric.md) v1.0。四个维度：Delegation Intensity、Cognitive Vigilance、Cognitive Reallocation、Zone Classification（Zone 1 / Zone 2 / Zone 3）。理论依据为 Wang, S., & Zhang, H. (2026). "Pedagogical partnerships with generative AI in higher education: how dual cognitive pathways paradoxically enable transformative learning." *International Journal of Educational Technology in Higher Education*, 23:11. DOI [10.1186/s41239-026-00585-x](https://doi.org/10.1186/s41239-026-00585-x)。
- **Cross-model 分歧显式标示，不默默平均**：当 `ARS_CROSS_MODEL` 设定时，observer 于两个模型同时执行；若任一维度分差 > 2 分即标记为 `cross_model_divergence`。另提供 `ARS_CROSS_MODEL_SAMPLE_INTERVAL` 调控成本。
- **Short-stage guard**：stage 内使用者 turn < 5 时注入静态 `insufficient_evidence` 区块，不派发全模型 observer call。
- **反谄媚规范**：分数 ≥ 7 必须附具体对话 turn 引用；Zone 3 触发 re-audit；禁止鼓励性语言。
- `academic-pipeline` SKILL 版本：`3.3.0 → 3.4.0`。Suite 版本升至 `3.5.0`。新增 lint `scripts/check_collaboration_depth_rubric.py` 加 10 个测试。
...
- **三回合 user-override 阶梯**，自动注入 `disclosure_addendum` 到 manuscript。无法规避揭露。
- **Calibration 以透明公布取代硬门槛**，与 `task_type: open-ended` 自洽。
- **Upstream freshness CI** 侦测 PRISMA-trAIce 上游漂移（non-blocking）。
- **长时间 session 文件**：Material Passport 作为跨 session 续跑机制。

### v3.3.6 (2026-04-15) — README 精简 + ARCHITECTURE 文件

- 新增 `docs/ARCHITECTURE.md` 作为 pipeline 结构的单一来源（流程、矩阵、资料存取、依赖图、品质闸门、模式）。透过 PR #18 合并入 main。
- 新增 `docs/SETUP.md` / `docs/SETUP.zh-TW.md`（前置需求、API key、Pandoc/tectonic、跨模型验证、四种安装方式），以及 `docs/PERFORMANCE.md` / `docs/PERFORMANCE.zh-TW.md`（token 预算、建议 扩展智能体运行环境 设定）。README 以连结取代内嵌。
- 精简 README：移除 ASCII pipeline 图与 16 项 key-feature 清单（已被 ARCHITECTURE.md 取代）；Skill 详细资讯维持版本号锚点，读者跳到 ARCHITECTURE.md §3 看各 agent 名单。
- 注记：没有任何 skill 的功能变动，纯文件重构。suite version 升级至 `3.3.6`。

### v3.3.5 (2026-04-15)
- 新增 `benchmark_report.schema.json` 与 Material Passport 的 `repro_lock` 可选区块。两者都附 pattern 文件、lint、范例。首次引入正式的 Python 开发依赖清单（`requirements-dev.txt`）。

### v3.3.4 (2026-04-15) — README 更新纪录同步修补

- 同步 `README.md` 与 `README.zh-TW.md` 内嵌的 changelog 区块，补上原本缺漏的 `v3.3.3` 与 `v3.3.2` 发版摘要。
- 扩充 `scripts/check_spec_consistency.py`，之后 README changelog 若再漂移，CI 会直接 fail。
### v3.3.3 (2026-04-15) — Release Prep + Lint 强化

- 强化 SKILL frontmatter lint：缺少 closing `---` fence 时，现在会明确报错，不再把整份档案后半段误当成合法 YAML。
- frontmatter 若可被 YAML 解析但不是 mapping，现在会回报可读错误，而不是直接 crash。
- 修正中英文 README 中 post-publication audit showcase 连结失效的问题。
- 在 spec consistency check 补上 README 相对连结验证，之后 dead link 会直接让 CI fail。
- 将 DOCX 输出契约在文件中统一：直接产出 `.docx` 依赖 Pandoc，否则回退为 Markdown + 转换说明。
...
- 所有顶层 `SKILL.md` 新增 `metadata.data_access_level`，并以 `raw`、`redacted`、`verified_only` 为强制词汇。
- 所有顶层 `SKILL.md` 新增 `metadata.task_type`，并以 `open-ended`、`outcome-gradable` 为强制词汇。
- 为两个 metadata 栏位新增 lint script 与单元测试，并接到 GitHub Actions spec consistency workflow。
- 新增 `shared/ground_truth_isolation_pattern.md`，并在 `shared/handoff_schemas.md` 中补上对新词汇的说明入口。

### v3.3.1 (2026-04-14) — 规格一致性修补

- 同步 README、`.claude/CLAUDE.md`、`MODE_REGISTRY.md` 与各 `SKILL.md` 的 mode 数量与公开版本标示。
- 修正跨模型叙述：目前已实作的是诚信抽样查核与独立 DA critique；同侪审查第六位 reviewer 仍在规划中。
- 厘清 adaptive checkpoint 语意：SLIM checkpoint 仍然必须等待使用者明确确认。
- 再次明确化 Stage 2.5 与 Stage 4.5 诚信关卡不可跳过。
- 新增轻量 spec consistency 检查与 GitHub Actions workflow，避免后续再发生文件漂移。

### v3.3 (2026-04-09) — PaperOrchestra 启发的强化

整合 [PaperOrchestra](https://arxiv.org/abs/2604.05018)（Song, Song, Pfister & Yoon, 2026, Google）的技术。

- **Semantic Scholar API 验证** — Tier 0 程式化引用存在性查核。Levenshtein >= 0.70 标题比对、DOI 不符侦测、S2 ID 去重。API 不可用时优雅降级。
- **反泄漏协议** — 知识隔离指令优先使用 session 内材料，缺少的内容标记 `[MATERIAL GAP]` 而非用 LLM 记忆填补。降低 Mode 5/6 失败风险。
- **VLM 图表验证**（可选）— 用视觉模型闭环检查生成图表。10 项检核清单，最多 2 轮修正。
- **分数轨迹协议** — 跨修订轮次的逐维度评分差异追踪（7 个维度）。侦测退步（delta < -3）触发强制 checkpoint。
- **Stage 2 并行化** — 视觉化与论证建构可在大纲完成后并行执行。
- 新版本：deep-research v2.8、academic-paper v3.0、academic-pipeline v3.2

### v3.2 (2026-04-09) — Lu 2026 Nature 整合

整合 Lu 等人（2026，*Nature* 651:914-919）的研究洞见——第一个通过盲审的端到端全自动 AI 研究系统。

- **7 类 AI 研究失败模式检查清单** — 在 Stage 2.5/4.5 阻断管线：侦测实作错误、幻觉实验结果、取巧特征依赖、错误包装为发现、方法伪造、框架锁定。扩充现有 5 类引用幻觉分类。
- **Reviewer 校准模式**（academic-paper-reviewer v1.8）— opt-in 的 FNR/FPR/balanced accuracy 测量，使用者提供 gold set。5 次集成、跨模型预设开启、session 内强制附加信心揭露。
- **揭露模式**（academic-paper v2.9）— 针对特定期刊/会议的 AI 使用声明生成器。v1 涵盖 ICLR、NeurIPS、Nature、Science、ACL、EMNLP。
...
- **跨模型验证协议**（shared，可选）：用 GPT-5.4 Pro 或 Gemini 3.1 Pro 做诚信验证 30% 抽样跨模型检查与独立 DA critique。同侪审查第六位 reviewer 仍在规划中，尚未实作。设定 `ARS_CROSS_MODEL` 环境变数启用——未设定时零开销。完整设定指南见 `shared/cross_model_verification.md`。
