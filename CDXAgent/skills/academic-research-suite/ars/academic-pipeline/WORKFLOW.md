...
Academic-pipeline must behave like a command-driven orchestrator.

When this workflow is active:

1. Begin every response with a progress block before any stage details.
2. The progress block must state:
   - Active skill: `academic-pipeline`
   - Current stage
   - Current mode or entry path
   - Completed stages or checkpoints
   - What is being executed now
   - What remains before the next gate
   - Whether user confirmation is required
3. Keep the progress block detailed enough that the user can tell the pipeline
   state at a glance.
4. Preserve mandatory checkpoint behavior: progress reporting does not weaken
   integrity gates, review gates, or user-confirmation gates.
5. Treat stage transitions as explicit commands, not silent continuations.
6. At every FULL, SLIM, or MANDATORY checkpoint, update the progress block
   before presenting deliverables or asking for confirmation.
7. If the workflow is complete, report the final pipeline state and mark the
   progress block as complete.
...
- **Intent**: Invoke in a *fresh* 扩展智能体运行环境 session. Resuming within the same session that emitted the boundary provides no token savings and may drop still-live in-session context.
