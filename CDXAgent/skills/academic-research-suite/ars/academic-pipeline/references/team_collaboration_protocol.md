...
Guidelines for coordinating multi-person academic research teams using the pipeline. 扩展智能体运行环境 runs as a single-user session; this protocol documents the **human coordination layer** that wraps around pipeline executions.
...
- 扩展智能体运行环境 runs as a single-user session; this protocol documents the HUMAN coordination layer
