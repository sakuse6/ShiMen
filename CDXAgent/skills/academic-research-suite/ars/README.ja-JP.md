# 扩展智能体运行环境 向け Academic Research Skills
...
学術研究のための 扩展智能体运行环境 スキル統合スイート。研究から論文公開までの全工程をカバーします。

**30秒でインストール**（扩展智能体运行环境 CLI / VS Code / JetBrains、v3.7.0+）:
...
- [扩展智能体运行环境](https://docs.claude.com/en/docs/claude-code/setup)（最新版。プラグインパッケージングは最近のバージョンが必要）
...
**👉 [docs/SETUP.md](docs/SETUP.md)** — 完全ガイド: 扩展智能体运行环境 インストール、API キー設定、DOCX/PDF 用のオプション Pandoc/tectonic、クロスモデル検証（`ARS_CROSS_MODEL`）、5 つのインストール方法（Plugin、プロジェクトスキル、グローバルスキル、claude.ai Project、リポジトリクローン）。

**隔离智能体运行环境 を使用していますか?** 代わりに姉妹ディストリビューションをインストールしてください: [`Imbad0202/academic-research-skills-codex`](https://github.com/Imbad0202/academic-research-skills-codex) — 同じワークフローコンテンツ、`ars-*` エイリアスを持つ単一の `$academic-research-suite` スキルとしての Codex ネイティブパッケージング。

## パフォーマンス＆コスト

**👉 [docs/PERFORMANCE.md](docs/PERFORMANCE.md)** — モードごとのトークン予算、フルパイプライン見積り（15k 語の論文で約 $4-6）、推奨 扩展智能体运行环境 設定（Auto モード; Agent Team オプション）。
...
### v3.7.0 (2026-05-05) — 扩展智能体运行环境 プラグインパッケージング

> プラグインパッケージングアップグレード: ARS は `/plugin marketplace add Imbad0202/academic-research-skills` + `/plugin install academic-research-skills` 経由で 扩展智能体运行环境 CLI / VS Code / JetBrains 上に 1 行でインストール可能に。従来の `git clone + ~/.claude/skills/ へのシンボリックリンク` フローも引き続き動作 — 両トラックともファーストクラス。
...
- `docs/SETUP.md`（前提条件、API キー、Pandoc/tectonic、クロスモデル検証、インストール方法）と `docs/PERFORMANCE.md`（トークン予算、推奨 扩展智能体运行环境 設定）を追加。README はインライン化する代わりに両方にリンク。
