...
AI 编程智能体正在快速演进，越来越多本地运行环境与命令行工具不断涌现，但管理它们却很零散：终端窗口到处都是、会话日志深埋文件中、配置需要手动编辑 JSON。
...
通过插件架构，在隔离运行环境、扩展运行环境与第三方运行环境之间切换。支持跨项目并行运行多个对话，上下文互不干扰。
