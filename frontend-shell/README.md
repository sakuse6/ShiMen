...
AI coding agents are evolving rapidly, with more powerful local runtimes and command-line tools appearing every day. But managing them is fragmented: scattered terminals, buried session logs, hand-edited configs.
...
Switch smoothly between isolated, extended, and third-party agent runtimes via the plugin architecture. Run multiple sessions in parallel across projects without context interference.
...
- Currently supports isolated, extended, and third-party agent runtimes
...
| `~/.agent-runtime/` | Extended agent runtime data |
