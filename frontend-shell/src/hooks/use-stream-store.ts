...
const STREAM_RECOVERY_KEY = "lmentor.active-streams.v1";
...
class StreamStore {
  ...
  private restore(): void {
    try {
      const raw = window.sessionStorage.getItem(STREAM_RECOVERY_KEY);
      if (!raw) return;
      const saved = JSON.parse(raw) as { sessions?: SessionStreamState[]; aliases?: Array<[string, string]> };
      for (const state of saved.sessions ?? []) {
        if (state?.abortKey) this.sessions.set(state.abortKey, { ...state, isStreaming: true });
      }
      for (const [alias, canonical] of saved.aliases ?? []) {
        if (alias && canonical) this.aliases.set(alias, canonical);
      }
    } catch {
      window.sessionStorage.removeItem(STREAM_RECOVERY_KEY);
    }
  }

  private persist(): void {
    try {
      const sessions = [...this.sessions.values()].filter((state) => state.isStreaming);
      if (sessions.length === 0) {
        window.sessionStorage.removeItem(STREAM_RECOVERY_KEY);
        return;
      }
      window.sessionStorage.setItem(STREAM_RECOVERY_KEY, JSON.stringify({
        sessions,
        aliases: [...this.aliases.entries()],
      }));
    } catch {
      // Refresh recovery must never interrupt the active stream.
    }
  }
  ...
  start(canonicalId: string, pendingUserMessage: string | null): void {
    ...
    if (existing) {
      // A restored stream may have reached sessionStorage before its user
      // bubble was committed. Preserve the first non-empty local input.
      if (!existing.pendingUserMessage && pendingUserMessage) {
        this.sessions.set(key, { ...existing, pendingUserMessage });
        this.scheduleFlush();
      }
      return;
    }
  ...
  /** Reconnect a persisted in-flight stream after a browser refresh. */
  hydrate(canonicalId: string, pendingUserMessage: string | null, resolvedId?: string | null): void {
    const key = this.canonical(canonicalId);
    const previous = this.sessions.get(key) ?? emptyState(key, pendingUserMessage);
    const resolved = resolvedId || previous.resolvedId;
    this.sessions.set(key, {
      ...previous,
      pendingUserMessage: previous.pendingUserMessage || pendingUserMessage,
      resolvedId: resolved,
      isStreaming: true,
    });
    if (resolved && resolved !== key) this.aliases.set(resolved, key);
    this.scheduleFlush();
  }
  ...
  push(sid: string, chunk: StreamChunk): void {
    ...
    if (data.kind === "text_delta") {
      ...
      // Some compatible upstreams only emit a completed item. Keep the
      // activity visible even when its matching start event was omitted.
      if (!tools.some((tool) => tool.id === data.call_id)) {
        tools = [...tools, { id: data.call_id, name: "运行工具", input: {} }];
        content = [...content, { type: "tool_use", id: data.call_id, name: "运行工具", input: {} }];
      }
  ...
  /** Return the stable key shared by a pending id and its resolved session id. */
  resolveSessionKey(sid: string): string {
    return this.canonical(sid);
  }
