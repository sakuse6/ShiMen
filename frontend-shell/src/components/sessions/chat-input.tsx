...
import { Check, ChevronDown, KeyRound, Paperclip, Plus, Send, Square } from "lucide-react";
...
function buildAttachmentControlBlock(lines: string[]): string {
  return [
    "<!--LMENTOR_FILE_CONTEXT_BEGIN-->",
    "[Attached files]",
    "[Use the local file paths below as the non-image attachments for this turn.]",
    ...lines,
    "<!--LMENTOR_FILE_CONTEXT_END-->",
  ].join("\n");
}

function buildAttachmentReferenceBlock(lines: string[]): string {
  return [
    "<!--LMENTOR_ATTACHMENTS_BEGIN-->",
    "[Attached files]",
    "[Reference only: attached files kept for Lmentor preview and reopening.]",
    ...lines,
    "<!--LMENTOR_ATTACHMENTS_END-->",
  ].join("\n");
}
...
const ChatInputBase = forwardRef<HTMLTextAreaElement, ChatInputProps>(function ChatInput({
  ...
  const handleFileSelect = async () => {
    if (!allowFiles || disabled || sending || isStreaming) return;
    ...
    try {
      ...
      const newFiles: AttachedFile[] = paths.map((filePath, i) => {
        ...
        const local = Boolean(projectPath && isInsideProject(filePath, projectPath));
        const idx = files.length + i + 1;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const promptFileLines: string[] = [];
      const attachmentReferenceLines: string[] = [];
      ...
      if (allowFiles && files.length > 0) {
        ...
        for (const f of localFiles) {
          ...
          attachmentReferenceLines.push(`${f.label}: ${localPath}`);
          if (f.isImage) nativeImagePaths.push(localPath);
          else promptFileLines.push(`${f.label}: ${localPath}`);
        }
        ...
        if (filesToUpload.length > 0) {
          ...
          for (const s of saved) {
            const originalFile = filesToUpload[s.index];
            attachmentReferenceLines.push(`${s.label}: ${s.path}`);
            if (originalFile?.isImage) nativeImagePaths.push(s.path);
            else promptFileLines.push(`${s.label}: ${s.path}`);
          }
        }
        ...
        if (promptFileLines.length > 0) {
          controlBlocks.push(buildAttachmentControlBlock(promptFileLines));
        }
        if (attachmentReferenceLines.length > 0) {
          controlBlocks.push(buildAttachmentReferenceBlock(attachmentReferenceLines));
        }
      }
      ...
      const pendingId = sessionId || activeSessionId;
      if (pendingId) {
        streamStore.drop(pendingId);
      }
      setSendError(err instanceof Error ? err.message : "\u6d88\u606f\u53d1\u9001\u5931\u8d25\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5\u3002");
      setSending(false);
      setActiveSessionId(null);
    }
  };
  ...
  return (
    <div className={cn("mx-auto w-full max-w-[var(--message-content-max-width)] px-4 pb-4 pt-2", containerClassName)}>
      ...
      >
        ...
        {notice ? (
          <div className="mx-3 mt-1 rounded-xl border border-[var(--icon-action)]/25 bg-[var(--icon-action)]/8 px-3 py-2 text-xs text-foreground shadow-sm">
            {notice}
          </div>
        ) : null}
