  ...
  return (
    <div className="markdown-prose overflow-hidden">
      <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]} components={markdownComponents}>
        {normalizeArtifactMarkdown(normalizeMermaidMarkdown(text))}
      </ReactMarkdown>
    </div>
  );
});
  ...
  for (const messageIndex of messageIndices) {
    ...
    message.content.forEach((block, blockIndex) => {
      if (block.type === "tool_use") {
        ...
        if (isInternalToolScaffolding(block.name, block.input)) return;
        pendingTools.push({
          id,
          toolName: block.name,
    ...
    <div className="w-full">
      <div className="max-w-full min-w-0 flex flex-col">
        <div className="flex items-center gap-2 mb-0.5 text-[11px]">
          <span className="font-medium text-muted-foreground">{assistantLabel}</span>
        </div>
...
export const MessageView = memo(function MessageView({
  ...
  }, [currentOcc, messages, renderingQuery, resolvedAssistantLabel, resultMap, searchState.offsets]);
