...
import { useRosterAgent } from "@/agents";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import rehypeHighlight from "rehype-highlight";
import { useSessionStream } from "@/hooks/use-stream-store";
import { InlineImageDisplay, InlineImages, isSupportedImagePath, stripImagePrompt } from "./inline-image";
import { MermaidCode, MermaidPre, normalizeMermaidMarkdown } from "./mermaid-diagram";
import { ArtifactMarkdownLink, normalizeArtifactMarkdown } from "./artifact-file";
import { ToolGroup, classifyToolName, isInternalToolScaffolding } from "@/components/observability/tool-call-card";
...
function displayRosterUserName(name: string | null | undefined, fallback = "来访者") {
  const normalized = String(name || "").trim();
  return normalized || fallback;
}

export const StreamingMessage = memo(function StreamingMessage({ sessionId, isComplete = false, userMessage, scrollContainerRef, assistantLabel }: StreamingMessageProps) {
  const state = useSessionStream(sessionId);
  const { t } = useTranslation();
  const { active, userName } = useRosterAgent();
  const displayText = state?.text ?? "";
  const thinkingText = state?.thinking ?? "";
  const errorText = state?.error ?? "";
  const toolUses = state?.tools ?? [];
  const visibleToolUses = toolUses.filter((tool) => !isInternalToolScaffolding(tool.name, tool.input));
  const hiddenToolIds = new Set(toolUses.filter((tool) => !visibleToolUses.includes(tool)).map((tool) => tool.id));
  const content = state?.content ?? [];
  const visibleContent = content.filter((block) => (
    block.type === "tool_use" ? !hiddenToolIds.has(block.id) : block.type === "tool_result" ? !hiddenToolIds.has(block.tool_use_id) : true
  ));
  const resolvedUserMessage = userMessage === undefined ? state?.pendingUserMessage ?? undefined : userMessage ?? undefined;
  const resolvedAssistantLabel = String(assistantLabel || active?.name || t("sessions.assistant")).trim() || t("sessions.assistant");
  const resolvedUserLabel = displayRosterUserName(userName, t("sessions.user"));
  ...
  useEffect(() => {
    ...
    scrollToBottom();
  }, [displayText, thinkingText, errorText, visibleToolUses.length, visibleContent.length, scrollToBottom, isComplete]);

  const hasBubbleContent = visibleContent.length > 0 || displayText.length > 0 || thinkingText.length > 0 || errorText.length > 0;
  const hasContent = hasBubbleContent || visibleToolUses.length > 0;

  // Map streaming tool calls to the same card model used by persisted messages.
  const streamToolCalls: ToolCall[] = visibleToolUses.map((tool, i) => ({
  ...
  const renderItems = buildStreamRenderItems(visibleContent, streamToolCalls);
    ...
    <div className="mx-auto w-full max-w-[var(--message-content-max-width)] space-y-2 px-4 py-3">
        ...
        <div className="w-full flex justify-end">
          <div className="max-w-[88%] min-w-0 flex flex-col items-end">
            <div className="flex items-center gap-2 mb-0.5 text-[11px]">
              <span className="font-medium text-muted-foreground">{resolvedUserLabel}</span>
            </div>
      ...
      <div className="w-full">
        <div className="max-w-full min-w-0 flex flex-col">
          <div className="flex items-center gap-2 mb-0.5 text-[11px]">
            <span className="font-medium text-muted-foreground">{resolvedAssistantLabel}</span>
          </div>
            ...
            <div className="space-y-1.5">
              {hasContent && (
                <div className="rounded-xl bg-[var(--message-assistant-bg)] text-[var(--message-assistant-fg)] px-3 py-2 overflow-hidden min-w-0 max-w-full space-y-2">
                    ...
                    if (item.block.type === "text") {
                      return (
                        <div key={`text-${idx}`} className="markdown-prose overflow-hidden">
                          ...
                          >
                            {normalizeArtifactMarkdown(normalizeMermaidMarkdown(item.block.text))}
                          </ReactMarkdown>
                        </div>
                      );
                    }
                  ...
                  {errorText && (
                    <div className="rounded-[6px] border border-destructive/30 bg-destructive/10 px-2.5 py-2 text-sm text-destructive whitespace-pre-wrap break-all">
                      {errorText}
                    </div>
                  )}
                  {!isComplete && (
                    <div className="flex min-w-0 items-center gap-2 overflow-hidden text-sm text-muted-foreground">
                      ...
                      <span className="processing-marquee">
                        {visibleToolUses.length > 0 ? t("sessions.toolCalling") : t("sessions.processing")}
                      </span>
                    </div>
                  )}
                </div>
...
const markdownComponents = {
  ...
  img(props: { src?: string; alt?: string }) {
    const src = props.src || "";
    if (isSupportedImagePath(src)) {
      return <InlineImageDisplay path={src} />;
    }
    return <img src={src} alt={props.alt || ""} className="max-w-full rounded border border-border/50" />;
  },
};
