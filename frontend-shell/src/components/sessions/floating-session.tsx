...
export function FloatingSessionView() {
  ...
  const sessionName = resolveSessionTitle([decodeURIComponent(params.get("name") || ""), sessionId], t("sessions.newChat"));
    ...
    listen<AgentStreamChunk[] | AgentStreamChunk>("agent-event", (event) => {
      ...
      for (const chunk of chunks) {
        ...
        if (chunk.data.kind === "session_resolved") {
          ...
          setLastText(current.length > 60 ? `...${current.slice(-57)}` : current);
    ...
    <div className="flex flex-col h-screen w-screen select-none overflow-hidden bg-background border border-border/50 rounded-lg">
      ...
      >
        ...
        <button
          ...
          title={t("sessions.floatingRestore", "恢复会话")}
        >
      ...
      >
        ...
        <span className="text-[11px] text-muted-foreground truncate" title={agentName}>
          {agentName}
        </span>
      </div>
