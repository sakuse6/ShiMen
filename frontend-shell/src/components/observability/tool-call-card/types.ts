...
/** Hide runtime scaffolding used only to load or encode skill instructions. */
export function isInternalToolScaffolding(name: string, input?: unknown): boolean {
  const normalizedName = String(name || "").trim().toLowerCase().replace(/[\s_-]+/g, "");
  if (["node", "getcontent", "setcontent"].includes(normalizedName)) return true;

  if (!input || typeof input !== "object") return false;
  const command = String((input as Record<string, unknown>).command || "").trim();
  if (!command) return false;
  return /(?:node(?:\.exe)?\s+-e[\s\S]*readFileSync[\s\S]*SKILL\.md|(?:get-content|set-content)\b[\s\S]*SKILL\.md)/i.test(command);
}

export function classifyToolName(name: string): ToolKind {
  const n = name.toLowerCase();
  if (n === "skill_index" || n.startsWith("skill:")) return "skill";
  ...
  if (n === "bash" || n === "shell" || n === "exec" || n === "shell_command" || n === "execute_command" || n === "run_shell_command" || n.endsWith(".shell_command")) return "shell_exec";
