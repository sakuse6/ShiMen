...
export const ToolCallCard = memo(function ToolCallCard({ call }: { call: ToolCall }) {
  ...
  const skillName = call.kind === "skill"
    ? (call.toolName === "skill_index" ? "已激活 Skill 索引" : call.toolName.replace(/^skill:/i, ""))
    : "";
  const cardLabel = skillName || path || call.toolName;
  const shortPath = cardLabel.length > 60 ? "..." + cardLabel.slice(cardLabel.length - 55) : cardLabel;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1 font-mono text-[0.95em] truncate text-[var(--color-foreground)]" title={cardLabel}>
          {shortPath}
        </span>
        {diff && (
